  ...
  return (
    <div className="grid grid-cols-3 gap-3 rounded-lg bg-zinc-900/80 p-3">
      ...
      <div className="text-center">
        ...
        <div className="text-xs text-zinc-500">{t("landing.countries")}</div>
      </div>
      <div className="text-center">
        <div className="text-lg font-bold text-zinc-200">{daysOfConflict}</div>
        <div className="text-xs text-zinc-500">{t("landing.days")}</div>
      </div>
    </div>
  );
}
    ...
    <div className="space-y-4 rounded-xl border border-zinc-800 bg-[#111] p-4 sm:space-y-5 sm:p-6">
      ...
      <div className="rounded-lg border border-amber-900/40 bg-amber-950/20 p-3">
        <p className="text-xs leading-relaxed text-amber-200/70">
          <span className="font-semibold">{t("landing.contentWarning")}</span>{" "}
          {t("landing.contentWarningText")}
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-1.5 text-center text-xs text-zinc-500 sm:gap-2">
        <div className="rounded bg-zinc-900/60 px-1.5 py-2 sm:p-2">
          <div className="font-medium text-zinc-300">{t("landing.mapFeature")}</div>
          <div className="hidden sm:block">{t("landing.mapDesc")}</div>
        </div>
        <div className="rounded bg-zinc-900/60 px-1.5 py-2 sm:p-2">
          <div className="font-medium text-zinc-300">{t("landing.askAiFeature")}</div>
          <div className="hidden sm:block">{t("landing.askAiDesc")}</div>
        </div>
        <div className="rounded bg-zinc-900/60 px-1.5 py-2 sm:p-2">
          <div className="font-medium text-zinc-300">{t("landing.feedFeature")}</div>
          <div className="hidden sm:block">{t("landing.feedDesc")}</div>
        </div>
      </div>
      ...
      <div className="text-center text-[11px] leading-relaxed text-zinc-500 sm:text-xs">
        <span className="font-medium">{eventCount} {t("landing.verifiedEventsFrom")}</span> {t("landing.from")}{" "}
  ...
  if (newEventCount === 0) {
    return (
      <div className="space-y-4 rounded-xl border border-zinc-800 bg-[#111] p-6 text-center">
        <h2 className="text-lg font-bold text-zinc-200">{t("landing.welcomeBack")}</h2>
        <p className="text-sm text-zinc-500">
          {t("landing.noNewEvents")} ({timeAgo}).
        </p>
        <p className="text-xs text-zinc-500">{t("landing.continuing")}</p>
      </div>
    );
  }
    ...
    <div className="space-y-4 rounded-xl border border-zinc-800 bg-[#111] p-6">
      ...
      <div className="rounded-lg bg-zinc-900/60 p-3">
        <div className="mb-2 text-xs font-semibold uppercase tracking-wider text-zinc-500">
          {newEventCount} {newEventCount !== 1 ? t("landing.newEvents") : t("landing.newEvent")}
        </div>
      ...
      <label className="flex cursor-pointer items-center justify-center gap-2 text-xs text-zinc-500">
        ...
        />
        {t("landing.skipBriefing")}
      </label>
    </div>
  );
}

function formatTimeAgo(t: (key: string, params?: Record<string, string | number>) => string, date: Date): string {
  const now = Date.now();
