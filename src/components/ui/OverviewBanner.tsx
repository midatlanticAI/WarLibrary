...
export default function OverviewBanner({ events }: OverviewBannerProps) {
  ...
  return (
    <div className="border-b border-zinc-800 bg-[#0e0e0e]">
      {/* ── Mobile: thin scrollable stats strip ── */}
      <div className="flex items-center gap-0 overflow-x-auto sm:hidden" style={{ scrollbarWidth: "none", WebkitOverflowScrolling: "touch" }}>
        <MobileStat label="DAY" value={String(stats.daysOfConflict)} color="text-zinc-100" />
        <StatDivider />
        <MobileStat label="KILLED" value={`${(stats.totalKilled / 1000).toFixed(1)}k+`} color="text-red-400" />
        <StatDivider />
        <MobileStat label="EVENTS" value={String(stats.totalEvents)} color="text-zinc-200" />
        <StatDivider />
        <MobileStat label="COUNTRIES" value={String(stats.countries)} color="text-blue-400" />
        <StatDivider />
        <MobileStat label="AIRSTRIKES" value={String(stats.airstrikes)} color="text-red-300" />
        <StatDivider />
        <MobileStat label="MISSILES" value={String(stats.missiles)} color="text-orange-400" />
        <StatDivider />
        <MobileStat label="DRONES" value={String(stats.drones)} color="text-yellow-400" />
        {stats.recentEvents > 0 && (
          <>
            <StatDivider />
            <MobileStat label="24H" value={String(stats.recentEvents)} color="text-green-400" />
          </>
        )}
      </div>
        ...
        {!collapsed && (
          <div className="space-y-3 px-4 pb-3">
            ...
            <div className="grid grid-cols-7 gap-2">
              ...
              <StatChip label="Civilian" value={String(stats.civilianImpactEvents)} color="text-amber-400" />
...
function MobileStat({
  ...
  return (
    <div className="flex shrink-0 flex-col items-center px-3 py-1.5">
...
function StatDivider() {
  return <div className="h-5 w-px shrink-0 bg-zinc-800" />;
}
